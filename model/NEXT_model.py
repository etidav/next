...
class next_model_no_ext_signal_FC_el(NEXT):
    """
    Class defining a next model with gaussian emission laws and discrete hidden states.
    """
...
#         dense_layers1_mu = tf.keras.layers.Dense(512)(flatten_layer)
        dense_layer3_mu = tf.keras.layers.Dense(self.horizon, dtype=tf.float64)(
            flatten_layer
        )
#         dense_layers1_sigma = tf.keras.layers.Dense(512)(flatten_layer)
        dense_layer3_sigma = tf.keras.layers.Dense(self.horizon, dtype=tf.float64)(
            flatten_layer
        )
        activation_layer3 = (
...
class next_model_no_ext_signal_FC_posterior(NEXT):
    """
    Class defining a next model with gaussian emission laws and discrete hidden states.
    """
...
class next_model_no_ext_signal_FC_full_posterior(NEXT):
    """
    Class defining a next model with gaussian emission laws and discrete hidden states.
    """
...
class next_model_no_ext_signal_full_FC(NEXT):
    """
    Class defining a next model with gaussian emission laws and discrete hidden states.
    """
...
class next_model_no_ext_signal_FC(NEXT):
    ...
    def define_hidden_state_prior_model(self) -> tf.keras.Model:
        ...
        """
        input_layer0 = tf.keras.Input((self.past_dependency, 2))
        dense_layer0 = tf.keras.layers.Dense(1)(input_layer0)
        flatten_layer0 = tf.keras.layers.Flatten()(dense_layer0)
        input_layer1 = tf.keras.Input((self.horizon, self.K))
        dense_layer1 = tf.keras.layers.Dense(1)(input_layer1)
        flatten_layer1 = tf.keras.layers.Flatten()(dense_layer1)
        concat_layer = tf.keras.layers.concatenate([flatten_layer0, flatten_layer1], axis=1)
        dense_layer = tf.keras.layers.Dense(100, activation='relu')(concat_layer)
        dense_layer0 = tf.keras.layers.Dense(self.horizon * self.K, dtype=tf.float64)(
            dense_layer
        )
        reshape_layer0 = tf.keras.layers.Reshape((self.horizon, self.K))(dense_layer0)
...
class next_model_with_ext_signal_FC(NEXT):
    """
    Class defining a next model with gaussian emission laws and discrete hidden states.
    """
...
class next_model_no_ext_signal_ref(NEXT):
    """
    Class defining a next model with gaussian emission laws and discrete hidden states.
    """
