...
class NEXT(ABC, tf.keras.Model):
    ...
    def compute_eval_metric(
        ...
        y_signal_norm = tf.math.divide_no_nan(
            sampled_y_signal - y_signal_mean, y_signal_std
        )
        w_signal_norm = tf.math.divide_no_nan(
            sampled_w_signal - y_signal_mean, y_signal_std
        )
        eval_loss = self.compute_elbo(
            y_signal_norm[:, self.past_dependency :],
            y_signal_norm[:, : self.past_dependency],
            w_signal_norm[:, : self.past_dependency],
            time_index,
            use_uniform_prior,
            None,
        )
    ...
    def predict(
        ...
        emission_laws_mu_renorm = emission_laws_mu * tf.expand_dims(
            y_past_std, axis=2
        ) + tf.expand_dims(y_past_mean, axis=2)
        prior_probabilities = self.compute_prior_probabilities(
            w_past, y_past, time_index
        )
        final_prediction_renorm = tf.math.reduce_sum(prior_probabilities*emission_laws_mu_renorm, axis=2)
        ...
        emission_laws_simulations_renorm = (
            emission_laws_simulations * y_past_std + y_past_mean
        )

        final_simulation_renorm = tf.math.reduce_sum(
            emission_laws_simulations_renorm * hidden_state_simulation_one_hot, axis=2
        )

        result = {}
        result["y_pred_mean"] = final_prediction_renorm.numpy()
        result["y_emission_law_mean"] = emission_laws_mu_renorm.numpy()
        result["prior_probabilities"] = prior_probabilities.numpy()
        result["all_y_pred"] = final_simulation_renorm.numpy()
        result["all_y_emission_law"] = emission_laws_simulations_renorm.numpy()
